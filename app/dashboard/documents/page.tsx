...
import { CalendarDays, Plus, Trash2, ChevronLeft, ChevronRight, Clock3, X, RefreshCw } from 'lucide-react'
...
function getDateKey(d: Date) { return `${d.getFullYear()}-${pad2(d.getMonth()+1)}-${pad2(d.getDate())}` }
function formatKoreanDate(d: Date) { return `${d.getFullYear()}년 ${d.getMonth()+1}월` }

function calEventToItem(ev: CalEvent): ScheduleItem {
  const dt = ev.start ? new Date(ev.start) : new Date()
  const date = ev.allDay ? ev.start.slice(0, 10) : getDateKey(dt)
  const time = ev.allDay ? '00:00' : `${pad2(dt.getHours())}:${pad2(dt.getMinutes())}`
  return { id: ev.id, title: ev.title, description: ev.description, date, time }
}
...
export default function DocumentsPage() {
  ...
  const calendarCells = useMemo(() => {
    ...
    for (let i = 0; i < startOffset; i++) {
      ...
      cells.push({ key: `prev-${getDateKey(prevDate)}`, date: getDateKey(prevDate), dayNumber: prevDate.getDate(), inMonth: false })
    }
    ...
    while (cells.length % 7 !== 0) {
      ...
      cells.push({ key: `next-${getDateKey(nextDate)}`, date: getDateKey(nextDate), dayNumber: nextDate.getDate(), inMonth: false })
    }
    return cells
  }, [currentDate])
  ...
  const fetchEvents = useCallback(async () => {
    ...
    try {
      ...
      setItems(data.events.map(calEventToItem))
    } catch (e) {
  ...
  async function onCreateSchedule() {
    ...
    try {
      ...
      const created = calEventToItem(data.event)
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div className="border-4 border-black bg-white shadow-[6px_6px_0_black] p-5 flex-1 mr-4">
          <h2 className="text-3xl font-black text-black uppercase">Google Calendar</h2>
          <p className="text-sm font-bold text-black mt-1">선택일 기준 {selectedItems.length}개 일정</p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={() => fetchEvents()}
            className="bg-white border-4 border-black shadow-[4px_4px_0_black] px-4 py-4 font-black text-black hover:shadow-[6px_6px_0_black] hover:-translate-x-0.5 hover:-translate-y-0.5 transition-all"
            title="새로고침"
          >
      ...
      {error && (
        <div className="mb-4 border-4 border-black bg-[#FF6B6B] px-4 py-3 font-black text-black flex items-center justify-between">
          <span>{error}</span>
          <button onClick={() => setError(null)} className="ml-2"><X size={14} strokeWidth={2.5} /></button>
        </div>
      )}

      {loading ? (
        <div className="border-4 border-black bg-white shadow-[4px_4px_0_black] p-8 text-center">
          <p className="font-black">구글 캘린더 로딩 중...</p>
        </div>
      ) : (
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="col-span-2 bg-white border-4 border-black shadow-[4px_4px_0_black] p-4">
            <div className="flex items-center justify-between mb-4">
              <button onClick={() => moveMonth(-1)} className="bg-white border-2 border-black p-2 hover:shadow-[2px_2px_0_black] transition-all">
            ...
            <div className="grid grid-cols-7 gap-2 mb-2">
              {days.map(day => (
                <div key={day} className="border-2 border-black bg-[#f5f0e8] py-2 text-center text-xs font-black uppercase">{day}</div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-7 gap-2">
              {calendarCells.map(cell => {
                ...
                return (
                  <button
                    key={cell.key}
                    onClick={() => { setSelectedDate(cell.date); setSelectedScheduleId(null); setError(null) }}
                    className={`min-h-28 border-2 border-black p-2 text-left transition-all ${selected ? 'bg-[#FFE500] shadow-[2px_2px_0_black]' : isToday ? 'bg-[#fff0a0]' : 'bg-white'} ${cell.inMonth ? '' : 'opacity-50'}`}
                  >
                    <div className={`text-sm font-black ${isToday ? 'underline decoration-2' : ''}`}>{cell.dayNumber}</div>
                    <div className="mt-1 space-y-1">
                      {items
                        ...
                        .slice(0, 2)
                        .map(item => (
                          <div key={item.id} className="border border-black bg-[#74C0FC] px-1 py-0.5 text-[10px] font-black leading-tight truncate">
                            {item.time !== '00:00' && `${item.time} `}{item.title}
                          </div>
                        ))}
                      {count > 2 && (
                        <div className="inline-block border border-black bg-white px-1 py-0.5 text-[10px] font-black">+{count - 2}개</div>
                      )}
                    </div>
                  </button>
                )
          ...
          <div className="bg-white border-4 border-black shadow-[4px_4px_0_black] p-4">
            <h3 className="font-black uppercase mb-3">일정 상세</h3>
              ...
              <div className="space-y-3">
                <div className="border-2 border-black p-2 max-h-48 overflow-auto space-y-1">
                  {selectedItems.map(item => (
                    ...
                    >
                      <p className="text-xs font-black truncate">{item.time !== '00:00' ? `${item.time} · ` : ''}{item.title}</p>
                    </button>
                  ))}
                </div>
                  ...
                  <div className="border-4 border-black bg-[#f5f0e8] p-4 min-h-40">
                    <div className="flex items-center justify-between gap-2 mb-3">
                      ...
                      <button
                        ...
                        disabled={deletingId === selectedSchedule.id}
                    ...
                    <p className="text-sm font-black flex items-center gap-1 mb-2">
                      ...
                      {selectedSchedule.date} {selectedSchedule.time !== '00:00' ? selectedSchedule.time : '종일'}
                    </p>
                    <div className="border-2 border-black bg-white p-3 min-h-24">
                      <p className="text-sm font-bold whitespace-pre-wrap">{selectedSchedule.description || '상세 설명이 없습니다.'}</p>
                    </div>
                  </div>
                ) : (
                  <div className="border-4 border-black bg-[#f5f0e8] p-4 min-h-40 flex items-center justify-center">
                    <p className="text-sm font-black text-center">일정을 클릭하면<br />상세 내용이 표시됩니다.</p>
                  </div>
                )}
              </div>
            )}
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-8">
          <div className="bg-white border-4 border-black shadow-[8px_8px_0_black] w-full max-w-md">
            <div className="flex items-center justify-between p-4 border-b-4 border-black bg-[#FFE500]">
              <span className="font-black uppercase">일정 등록</span>
            ...
            <div className="p-5 space-y-3">
              ...
              <div className="flex gap-2">
                <button onClick={onCreateSchedule} disabled={saving}
                  className="bg-[#69DB7C] border-4 border-black px-5 py-2 font-black uppercase disabled:opacity-50">
                  {saving ? '저장 중...' : '저장'}
                </button>
                <button onClick={() => setShowCreate(false)} className="bg-white border-4 border-black px-5 py-2 font-black uppercase">
                  취소
                </button>
              </div>
            </div>
